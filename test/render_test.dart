...
String anthropicLine(String text) => 'data: ${jsonEncode({
...
void main() {
  ...
  group('replay', () {
    test('yields the object as it fills in, not just at the end', () async {
      ...
      final states = await replay(readChunks(lines), Provider.anthropic)
          .toList();
    ...
    test('intermediate states only ever grow toward the final object',
        () async {
    ...
    test('an empty recording yields nothing rather than throwing', () async {
      final states =
          await replay(readChunks(const Stream.empty()), Provider.anthropic)
              .toList();
